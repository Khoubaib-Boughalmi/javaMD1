...
public class Program {
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < users.length && users[i] != null; i++) {
      System.out.println(users[i].getName() + " " + users[i].getIdentifier() + " has a balance of "
          + transactionsService.getUserBalance(users[i]));
    }
    try {
    ...
    try {
      ...
      transactionsService.removeTransferTransaction(sed, sedTransactions[0].getIdentifier());
    } catch (Exception e) {
      System.out.println(e.getMessage());
    }
    ...
    transactionsService.checkTransferTranscationValidity();
